...
class IndexController extends Controller
{
    ...
    {
        $arr = $_POST;
        trace('回调开始标记_post:'.json_encode($arr),'info_ali_notice_log');
        $config = ConfigController::getConfig();
        //> 校验
        //> 1.商户需要验证该通知数据中的out_trade_no是否为商户系统中创建的订单号，
        //> 2.判断total_amount是否确实为该订单的实际金额（即商户订单创建时的金额），
        # TODO wyh 20201029
        //$arr['out_trade_no'] = intval(explode('a',$arr['out_trade_no'])[0]);
        $where = array(
            'id' => $arr['out_trade_no'],
    ...
    private function orderHandle($data)
    {
        ...
        $up_data['payment'] = 'AliPayH5';//支付网关名称
    ...
    public function return_handle()
    {
        ...
        if($this->aliCheck($_GET)){
            ...
            //redirect('http://f.test.idcsmart.com/#/order');
