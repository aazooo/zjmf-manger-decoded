 ...
 */
namespace gateways\ali_pay\pagepay\service;
...
class AlipayTradeService {
	...
	function pagePay($builder,$return_url,$notify_url,$ispage=false) {
        ...
        // 首先调用支付api
		$response = $this->aopclientRequestExecute ($request,$ispage);
		return $response;
	}
...
//			$result = $aop->Execute($request);
	...
	function writeLog($text) {
		...
		//$text = characet ( $text );
		file_put_contents ( dirname(dirname ( __DIR__ ))."/log/log.txt", date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
	}
}
