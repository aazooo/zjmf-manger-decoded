...
function nokvm_CreateAccount($params){
	...
	$post_data['username'] = ($params['user_info']['phonenumber'] ?: $params['user_info']['email']) ?: $params['user_info']['username'];
...
function nokvm_TerminateAccount($params){
	...
	$res = nokvm_Curl($url, $post_data, 30, 'DELETE');
	if(isset($res['code']) && $res['code'] == 0){
		// 删除成功后
		$customid = Db::name('customfields')
					->where('type', 'product')
...
function nokvm_ChangePackage($params){
    ...
    if(empty($exist)){
        $data = [
            'fieldid'=>$customid,
            'relid'=>$params['hostid'],
            'value'=>$res['data']['id']??$vserverid,
            'create_time'=>time()
        ];
        Db::name('customfieldsvalues')->insert($data);
    }else{
        Db::name('customfieldsvalues')->where('id', $exist['id'])->update(['value'=>$res['data']['id']??$vserverid]);
    }
    # 结束
...
// function nokvm_TrafficUsage($params, $start, $end){
// 	$vserverid = nokvm_GetServerid($params);
// 	if(empty($vserverid)){
// 		$result['status'] = 400;
// 		$result['msg'] = '用量获取失败';
...
// 	$url = nokvm_GetUrl($params, '/api/virtual_monitoring/'.$vserverid, $sign);
// 	$res = nokvm_Curl($url, [], 10, 'GET');
// 	if(is_numeric($res['code']) && $res['code'] == 0){
// 		$result['status'] = 200;
// 		$result['data'] = [];
// 		$traffic = [];
// 		foreach($res['data'] as $k=>$v){
...
// 			$date = date('Y-m-d', strtotime($v['CreatedAt']));
// 			if(str_replace('-', '', $date) < str_replace('-', '', $start) || str_replace('-', '', $date) > str_replace('-', '', $end)){
// 				continue;
// 			}
// 			if($k == 0 || $k == count($res['data']) - 1){
// 				$traffic[$date] += ($v['Network']['in'] + $v['Network']['out']) * 60;
// 			}else{
// 				$traffic[$date] += ($v['Network']['in'] + $v['Network']['out']) * 120;
// 			}
// 		}
    	...
    	$v['server_password'] = aesPasswordDecode($v['server_password']);
  		...
  		if(is_numeric($res['data']['flow']['code']) && $res['data']['flow']['code'] == 0){
  			...
  			if($v['domainstatus'] == 'Active' && $update['bwlimit'] > 0 && $update['bwusage'] > $update['bwlimit']){
                $result = $host->suspend($v['id'], 'flow', '用量超额');
                #任务队列-数据
                $logic_run_map = new RunMap();
                $model_host = new HostModel();
                $data_i = [];
                $data_i['host_id'] = $v['id'];
                $data_i['active_type_param'] = [$v['id'], 'flow', '用量超额',0];//suspend 方法所有参数
                $is_zjmf = $model_host->isZjmfApi($data_i['host_id']);
                if ($result['status']==200){
                    $data_i['description'] = " 流量使用超额 - 暂停 Host ID:{$data_i['host_id']}的产品成功";
                    if ($is_zjmf)$logic_run_map->saveMap($data_i,1,400,2);
                    if (!$is_zjmf)$logic_run_map->saveMap($data_i,1,100,2);
                }else{
                    $data_i['description'] = " 流量使用超额 - 暂停 Host ID:{$data_i['host_id']}的产品失败：{$result['msg']}";
                    if ($is_zjmf)$logic_run_map->saveMap($data_i,0,400,2);
                    if (!$is_zjmf)$logic_run_map->saveMap($data_i,0,100,2);
                }

  			}
  			$suspendreason = explode('-', $v['suspendreason'])[0];
  			// 解除暂停
  			if($v['domainstatus'] == 'Suspended' && ($update['bwlimit'] == 0 || $update['bwusage'] < $update['bwlimit']) && ($suspendreason == 'flow' || $suspendreason == '用量超额')){
                $result = $host->unsuspend($v['id'], 0, '', true);
                ...
                $data_i = [];
                $data_i['host_id'] = $v['id'];
                $data_i['active_type_param'] = [$v['id'], 0, '', 1];//unsuspend 方法所有参数
                $is_zjmf = $model_host->isZjmfApi($data_i['host_id']);
                if ($result['status']==200){#对接上游
                    $data_i['description'] = " 满足超额解除暂停条件 - 解除暂停 Host ID:{$data_i['host_id']}的产品成功";
                    if ($is_zjmf)$logic_run_map->saveMap($data_i,1,400,3);
                    if (!$is_zjmf)$logic_run_map->saveMap($data_i,1,100,3);
                }else{
                    $data_i['description'] = " 满足超额解除暂停条件 - 解除暂停 Host ID:{$data_i['host_id']}的产品失败：{$result['msg']}";
                    if ($is_zjmf)$logic_run_map->saveMap($data_i,0,400,3);
                    if (!$is_zjmf)$logic_run_map->saveMap($data_i,0,100,3);
                }
...
function nokvm_FlowPacketPaid($params){
	...
	if($params['configoptions']['flow_limit'] > 0){
		...
		nokvm_Sync($params);
		// 如果超额暂停后,解除暂停
		$suspendreason = explode('-', $params['suspendreason'])[0];
		// 解除暂停
		if($params['domainstatus'] == 'Suspended' && ($suspendreason == 'flow' || $suspendreason == '用量超额')){
			// 获取同步后用量
			$after = Db::name('host')
					->field('bwusage,bwlimit')
					->where('id', $params['hostid'])
					->find();
			if($after['bwlimit'] == 0 || $after['bwlimit'] > $after['bwusage']){
				$host = new \app\common\logic\Host();
                $result = $host->unsuspend($params['hostid'], 0, '', true);
                ...
                $data_i['active_type_param'] = [$params['hostid'], 0, '', 1];//unsuspend 方法所有参数
                $is_zjmf = $model_host->isZjmfApi($data_i['host_id']);
                if ($result['status']==200){#对接上游
                    $data_i['description'] = " 满足超额解除暂停条件 - 解除暂停 Host ID:{$data_i['host_id']}的产品成功";
                    if ($is_zjmf)$logic_run_map->saveMap($data_i,1,400,3);
