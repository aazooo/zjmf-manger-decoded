...
class App extends Container
{
    ...
    public function initialize()
    {
        ...
        if (!empty($this->config('app.root_namespace'))) {
            ...
            #TODO WYH 20210115 注册public\plugins\gateways\命名空间,gateways兼容两个目录
            Loader::addNamespace($this->config('app.root_namespace_other'));
        }
    ...
    public function init($module = '')
    {
        ...
        if (empty($module) || empty(static::$modulesInited[$module])) {
            ...
            if (is_file($path . 'init.php')) {
                ...
                # TODO wyh 加载客户定义钩子文件 20210113
                # 20210915 出现问题,被放入后门文件,先注释
                /*$dirs = array_map('basename', glob(WEB_ROOT . 'plugins/behavior/*', GLOB_ONLYDIR));
                foreach ($dirs as $dir){
                    $files = glob(WEB_ROOT . 'plugins/behavior/' . $dir . '/*');
                    if (!empty($files)){
    ...
    protected function containerConfigUpdate($module)
    {
        ...
        // 加载当前模块语言包
		$admin= ($module!="home".DIRECTORY_SEPARATOR) ? DIRECTORY_SEPARATOR .'admin' : "";
        $this->lang->load(CMF_ROOT . "public" .$admin . DIRECTORY_SEPARATOR . 'lang' . DIRECTORY_SEPARATOR . $this->request->langset() . '.php');
