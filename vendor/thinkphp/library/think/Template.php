...
class Template
{
    ...
    public function fetch($template, $vars = [], $config = [])
    {
        ...
        if ($template) {
            ...
            if (!$this->checkCache($cacheFile) || $_SESSION['view_tpl_debug']) {
                // 缓存无效 重新模板编译
                $common =$this->getTpl($template);
                $common =view_tpl_replace($common,"debug",$templateName);
                $content =view_tpl_replace($common,"tagdata");
    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        if ($this->config['layout_on']) {
            if (false !== strpos($content, '{__NOLAYOUT__}')) {
                ...
                if ($layoutFile) {
                    $common=file_get_contents($layoutFile);
                    //$common =$this->getTpl($layoutFile);
                    // 替换布局的主体内容
                    $content = str_replace($this->config['layout_item'], $content, view_tpl_replace($common,"tagdata"));
                }
            }
        } else {
            $content = str_replace('{__NOLAYOUT__}', '', $content);
        ...
        //token替换
        preg_match_all("/<\s*form.*?>/is",$content,$inForm);
        foreach($inForm[0] as $k=>$form){
            $formMethod=strtolower(str_replace(" ","",$form));
            if(stripos($formMethod,'method="post"')!==false){
                $content=str_replace($form,$form.'<input type="hidden" name="token" value="{$Token}">',$content);
            }
        }
...
//        # TODO wyh 20210301 增加资源变量
//        $shd_replace = [
//            '__STATIC__' => cmf_get_root() . '/' . adminAddress() . '/themes/' . adminTheme(),
//        ];
//        $replace = array_merge($replace,$shd_replace);
        ...
        if (preg_match($this->getRegex('layout'), $content, $matches)) {
            ...
            if (!$this->config['layout_on'] || $this->config['layout_name'] != $array['name']) {
                ...
                if ($layoutFile) {
                    ...
                    $common =$this->getTpl($layoutFile);
                    // 替换布局的主体内容
                    $content = str_replace($replace, $content,view_tpl_replace($common,"tagdata"));
                }
            }
        } else {
            $content = str_replace('{__NOLAYOUT__}', '', $content);
    ...
    private function parseTemplateName($templateName)
    {
        ...
        foreach ($array as $templateName) {
            ...
            if ($template) {
                // 获取模板文件内容
				if(strpos($template,"template not exists")!==false && $this->config['autoinclude']!==false){
					$common =$template;
				}else{
					$common = file_get_contents($template);
				}
                //$common =$this->getTpl($template);
                $common =view_tpl_replace($common,"debug",$templateName);
                $parseStr .=view_tpl_replace($common,"tagdata");
            }
        }
    ...
    private function parseTemplateFile($template)
    {
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            if ($this->config['view_base']) {
                ...
                # TODO WYH 20210301 shd自定义模板,不要模块
                if(VIEW_TEMPLATE_WEBSITE===true || VIEW_TEMPLATE_ADMIN == true) $module ='';
                $cartToClientarea=true;
                if(VIEW_TEMPLATE_DIRECTORY==="cart"&& VIEW_TEMPLATE_WEBSITE===true && stripos($template,$this->config['view_directory_themes'])!==false){
                    $cartToClientarea=false;
                    $this->config['view_base']=$this->config['view_themes'];
                }
            ...
            $file=$path . $template . '.' . ltrim($this->config['view_suffix'], '.');
            if(!file_exists($file) && VIEW_TEMPLATE_WEBSITE===true && $this->config['view_base_parent']){
                $file=$this->config['view_base_parent'] . $template . '.' . ltrim($this->config['view_suffix'], '.');
            }
            if(VIEW_TEMPLATE_DIRECTORY==="cart" && VIEW_TEMPLATE_WEBSITE===true && $cartToClientarea){
                /* $clientareaThemes = getClientareaThemes();
				$cart=$this->config['view_themes'].($this->config['view_directory_themes_parent']??$this->config['view_directory_themes']);
                $clientarea=$this->config['view_themes']."clientarea/".$clientareaThemes."/";
                $file=str_replace($cart,$clientarea,$file); */
				$cart=$this->config['view_themes'].($this->config['view_directory_themes_parent']??$this->config['view_directory_themes']);
				if(empty($this->config['view_header_and_footer'])){
					$clientareaThemes=configuration('clientarea_default_themes')?:'default';							
					$clientarea=$this->config['view_themes']."clientarea/".$clientareaThemes."/";					
					$file=str_replace($cart,$clientarea,$file);
					if(!file_exists($file)){
						$themeConfig=view_tpl_yaml($clientarea);
						if(!empty($themeConfig['config-parent-theme'])){
							$clientareaParent=$this->config['view_themes']."clientarea/".$themeConfig['config-parent-theme']."/";
							$file=str_replace($clientarea,$clientareaParent,$file);
						}
					}

				}else{
					$themes_templates=configuration('themes_templates');	
					$clientarea=$this->config['view_themes']."web/".$themes_templates."/";
					$file=str_replace(".tpl",".html",$file);
					$file=str_replace($cart,$clientarea,$file);
				}
		...
		return 'template not exists:' . $template;
    ...
    public function getTpl($template){
        if(request()->module() == 'install')  return file_get_contents($template);
        //if(VIEW_TEMPLATE_WEB===true) return file_get_contents($template);
        $adminAddress = adminAddress();
        $file=substr($template,0,strrpos($template,"/")); 
		if(file_exists($template)){
			$tplMain=file_get_contents($template);
			if($this->config['autoinclude']===false) return $tplMain;
		}else if($this->config['autoinclude']!==false){
			return 'template not exists:'.$template;
		}
        
		
        preg_match_all("/{\s*include.*?file\s*?=\s*?[\"\'](.*?)[\"\'].*?}/is", $tplMain,$inFiles);
        $header="";$footer="";$tplHeader="";$tplFooter="";$menu="";# 后台菜单
        foreach($inFiles[1] as $k=>$include){
            if(stripos($include,"/")!==false){
                if(stripos($include,"/header")!==false){
                    $header="include";
                }else if(stripos($include,"/footer")!==false){
                    $footer="include";
                }
            }else{
                if($include=="header"){
		...
		if(!empty($this->config['view_header_and_footer'])){
			//继承web头部
			$view_header_and_footer=$this->config['view_header_and_footer']."/common";
			$view_header_and_footer=$this->config['view_header_and_footer']."/common";
			$suffix="html";
		}else{
			//继承clientarea头部
			$view_header_and_footer="clientarea/{$clientareaThemes}";
			$suffix=VIEW_TEMPLATE_SUFFIX;
		}
		$param = \request() ->param();
        if(empty($header)){
            $headerFile=$file."/header.".VIEW_TEMPLATE_SUFFIX;
            if(VIEW_TEMPLATE_DIRECTORY==="cart" && VIEW_TEMPLATE_WEBSITE===true){
				//购物车头部引用
                $headerFile=CMF_ROOT."public/themes/{$view_header_and_footer}/header.".$suffix;
				
				if(!empty($param['theme'])){
					$themes=$param['theme'];
				}else{	
					$themes=configuration('clientarea_default_themes')?:'default';//会员中心主题
				}
				
				//有模板继承
				if($themes!==$clientareaThemes && empty($this->config['view_header_and_footer'])){
					
					if(file_exists(CMF_ROOT."public/themes/clientarea/{$themes}/header.".$suffix)){
						$headerFile=CMF_ROOT."public/themes/clientarea/{$themes}/header.".$suffix;
					}else{
						$headerFile=CMF_ROOT."public/themes/{$view_header_and_footer}/header.".$suffix;
					}
					//exit($headerFile);
				}else if(empty($this->config['view_header_and_footer'])){
				//没有模板继承
					$headerFile=CMF_ROOT."public/themes/clientarea/{$themes}/header.".$suffix;
				}
            }else if(VIEW_TEMPLATE_WEBSITE===true && $this->config['view_base_parent']){
                if(file_exists($this->config['view_base']."/header.".VIEW_TEMPLATE_SUFFIX)){
					$headerFile=$this->config['view_base']."/header.".VIEW_TEMPLATE_SUFFIX;
				}else{
					$headerFile=$this->config['view_base_parent']."/header.".VIEW_TEMPLATE_SUFFIX;
				}
            }else if(!file_exists($headerFile) && VIEW_TEMPLATE_ADMIN===true && $this->config['view_base_parent']){ # 后台

                $headerFile=CMF_ROOT."public/{$adminAddress}/themes/default/header.".VIEW_TEMPLATE_SUFFIX;
            }else if (!file_exists($headerFile) && VIEW_TEMPLATE_WEBSITE===true &&VIEW_TEMPLATE_HOME_PLUGINS===true){ # 插件前台头部文件引入
                $headerFile=CMF_ROOT."public/themes/clientarea/{$clientareaThemes}/header.".VIEW_TEMPLATE_SUFFIX;
            }else if (!file_exists($headerFile) && VIEW_TEMPLATE_ADMIN_PLUGINS===true){ # 插件后台头部文件引入
                $headerFile=CMF_ROOT."public/{$adminAddress}/themes/default/header.".VIEW_TEMPLATE_SUFFIX;
            }
            if(file_exists($headerFile)){
				$tplHeader=file_get_contents($headerFile);
			}else if ($this->config['autoinclude']!==false && VIEW_TEMPLATE_DIRECTORY==="cart" && VIEW_TEMPLATE_WEBSITE===true){
				$tplHeader='template not exists:'.$headerFile;
			}
        }

		if(empty($footer)){
			$footerFile=$file."/footer.".VIEW_TEMPLATE_SUFFIX;
			...
			if(VIEW_TEMPLATE_DIRECTORY==="cart" && VIEW_TEMPLATE_WEBSITE===true){
				...
				$footerFile=CMF_ROOT."/public/themes/{$view_header_and_footer}/footer.".$suffix;
				if($themes!==$clientareaThemes && empty($this->config['view_header_and_footer'])){
					if(file_exists(CMF_ROOT."/public/themes/clientarea/{$themes}/footer.".$suffix)){
						$footerFile=CMF_ROOT."/public/themes/clientarea/{$themes}/footer.".$suffix;
					}else{
						$footerFile=CMF_ROOT."/public/themes/{$view_header_and_footer}/footer.".$suffix;
					}
				}else if(empty($this->config['view_header_and_footer'])){
				...
				if(file_exists($this->config['view_base']."/footer.".VIEW_TEMPLATE_SUFFIX)){
					$footerFile=$this->config['view_base']."/footer.".VIEW_TEMPLATE_SUFFIX;
				}else{
					$footerFile=$this->config['view_base_parent']."/footer.".VIEW_TEMPLATE_SUFFIX;
				}
				
			}else if(!file_exists($footerFile) && VIEW_TEMPLATE_ADMIN===true && $this->config['view_base_parent']){ # 后台
                $footerFile=CMF_ROOT."public/{$adminAddress}/themes/default/footer.".VIEW_TEMPLATE_SUFFIX;
            }else if (!file_exists($footerFile) && VIEW_TEMPLATE_WEBSITE===true &&VIEW_TEMPLATE_HOME_PLUGINS===true){ # 插件前台底部文件引入
                $footerFile=CMF_ROOT."public/themes/clientarea/{$clientareaThemes}/footer.".VIEW_TEMPLATE_SUFFIX;
            } else if (!file_exists($footerFile) && VIEW_TEMPLATE_ADMIN_PLUGINS===true){ # 插件后台头部文件引入
                $footerFile=CMF_ROOT."public/{$adminAddress}/themes/default/footer.".VIEW_TEMPLATE_SUFFIX;
            }
            if(file_exists($footerFile)){
				$tplFooter=file_get_contents($footerFile);
			}else if ($this->config['autoinclude']!==false && VIEW_TEMPLATE_DIRECTORY==="cart" && VIEW_TEMPLATE_WEBSITE===true){
				$tplFooter='template not exists:'.$footerFile;
			}
		}
        $tplMenu = '';
		if (empty($menu)){
            if(VIEW_TEMPLATE_ADMIN===true || (VIEW_TEMPLATE_DIRECTORY!='cart' && VIEW_TEMPLATE_DIRECTORY!='clientarea' && VIEW_TEMPLATE_DIRECTORY!='web' && VIEW_TEMPLATE_ADMIN_PLUGINS===true)){ # 后台
                $menu = CMF_ROOT."public/{$adminAddress}/themes/default/menu." . VIEW_TEMPLATE_SUFFIX;
                $tplMenu = file_get_contents($menu);
            }
        }

		return $tplHeader.$tplMenu.$tplMain.$tplFooter;
	}
